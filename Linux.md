...
vnc密码：6618aA	实例密码：1132126618aqAQ
...
外网面板地址: http://116.62.172.184:8888/0f0b360a
内网面板地址: http://172.31.171.152:8888/0f0b360a
username: xg22prmj
password: 647069c1
...
输入没有返回错误就代表操作成功。没有消息就是好消息。

> 开机登入

开机会启动许多程序。它们在windows叫做“服务”（service），在Linux就叫做“守护进程”（daemon）。

开机成功后，他会显示一个文本登入界面，这个界面就是我们经常看到的登入界面，在这个登入界面中会提示用户输入用户名，而用户输入的用户将作为参数传给login程序来验证用户的身份，密码是不显示的，输完回车即可。
...
```bash
sync				# 将数据由内存同步到硬盘中（关机前确保数据不丢失使用）

shutdown			# 是最常用也是最安全的关机和重启命令，它会在关机之前调用fsck检查磁盘，其中-h和-r是最常用的参数：
	-h：			   # 停止系统服务并关机
	-r：  		   # 停止系统服务后重启
...
halt 				# 不理会目前系统状况下，进行硬件关机，一般不建议使用。相同shutdown -h now 和 poweroff
```
...
![image-20200814210422369](Linux.assets/image-20200814210422369.png)

树状目录结构：

![image-20200814212306016](Linux.assets/image-20200814212306016.png)
...
- /proc，这个目录是一个虚拟的目录，它是系统内存的映射，访问这个目录来获取系统信息（不用管）
...
- ==/www，存放服务器网站相关的资源，环境，网站的项目==（在服务器才有）
...
![image-20200814233645523](Linux.assets/image-20200814233645523.png)
...
![image-20200814232941956](Linux.assets/image-20200814232941956.png)
...
- tac      从最后一行开始显示，可以看出 tac 是 cat 的倒着写！
  ...
  ![image-20200815012048472](Linux.assets/image-20200815012048472.png)

- nl        显示的时候，顺道输出行号！（常用）

  ![image-20200815012141089](Linux.assets/image-20200815012141089.png)

- more  一页一页的显示文件内容，带（空格代表翻页，enter代表向下看一行，:f 行号）
- ==less     与more 类似，但是比 more 更好的是，他可以往前翻页！（上下键翻页，退出q命令，查找字符串 `/要查找的字符串`向下查询，向上查询用`？要查找的字符串n`，n代表寻找下一个，N上一个）==
- head  只看头几行 通过 -n 参数来控制显示几行  `head -n 20 文件名`
- tail     只看尾巴几行，相同head
...
![image-20200815125633081](Linux.assets/image-20200815125633081.png)
...
![image-20200815130944411](Linux.assets/image-20200815130944411.png)
...
查看 `cat /etc/group`即可看见刚才所创建的用户组
...
pstree		#也可以用这个，这个看的非常清楚
	-p 显示父id
	-u 显示用户组
```

![image-20200815225849416](Linux.assets/image-20200815225849416.png)
...
但是，我们平时写的java代码死循环了，可以通过结束进程，杀进程
...
- 解压（tomcat，启动并通过外网访问，发布网站）
- yum在先安装（docker：直接安装运行跑起来docker就可以）

### JDK安装

Linux jdk rpm：http://www.oracle.com/technetwork/java/javase/downloads/index.html
...
![image-20200816005445734](Linux.assets/image-20200816005445734.png)
...
LASSPATH=%JAVA_HOME%/lib;%JAVA_HOME%/jre/lib
PATH=$JAVA_HOME/bin:$JAVA_HOME/jre/bin
exprot PATH CLASSPATH JAVA_HOME
```

解压的需要配置环境变量，rpm的可以不用
...
如果是zip格式的使用` unzip + 文件名 即可解压`，注意文件名是全称
...
进入到tomcat的bin文件目录执行以上

![image-20200816132512775](Linux.assets/image-20200816132512775.png)



4、确保Linux的防火墙端口是开启的，如果是阿里云，需要保证阿里云的安全组是开放的！
